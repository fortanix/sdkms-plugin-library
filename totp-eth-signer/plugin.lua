...
-- Added the support for both types of notation.
...
function get_pub_key(hmac_seed_id, wallet_name, key_index)
    local path = get_path(wallet_name, key_index)
    local indices = parse_path(path)
    hmac_seed = assert(Sobject { kid =hmac_seed_id}, "hmac seed not found")
...
function sign_eth(hmac_seed_id, wallet_name, key_index, msg_hash)
...
function run(input)
    ...
    local all_ops = { op_sign, op_get_pub_key }
    ...
    local hmac_seed_id, err = expect_input_field(input, "hmac_seed_id", "string")
    if err ~= nil then return nil, err end
    ...
    if operation == op_get_pub_key then
        ...
        return get_pub_key(hmac_seed_id, wallet_name, key_index)
    end
    ...
    if operation == op_sign then
        ...
        if err ~= nil then
            return sign_eth(hmac_seed_id, wallet_name, key_index, msg_hash)
        end
