...
function extract_coordinates_from_ASN1_public_key(keybyte)
    return string.sub(keybyte, 49, 176)
end

-- return ec curve co-ordinate from private key
function get_point_coordinates_from_private_key(keybyte)
    ...
    local coordinate = extract_coordinates_from_ASN1_public_key(asn1_publicKey)
    return coordinate
end
...
function public_key_for_private_key(keybyte)
    local point = get_point_coordinates_from_private_key(keybyte)
...
function derive_new_child(parent_key, childIdx)
    ...
    if parent_key.version == PRIVATE_WALLET_VERSION then
        ...
        childKey.key = "00"..tostring(add_private_keys(string.sub(hmac, 1, 64), parent_key.key))
    else
   ...
   childKey.checksum = string.sub(get_check_sum(childKey), 1, 8)
