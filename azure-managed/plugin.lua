...
function wrap_traget_key(wrapping_key, target_key, target_key_type)
...
function run(input)
  ...
  if input.operation == 'configure' then
    ...
    if input.operation == 'create' then
      ...
      -- Wrap target key
      local wrapped_traget_key = wrap_traget_key(wrapping_key.result, target_key.result, input.key_type)
